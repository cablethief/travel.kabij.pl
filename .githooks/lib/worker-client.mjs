...
/** Lists an author's images: [{ path, url, size, uploaded, contentType }]. Public — no auth needed. */
export async function listImages({ workerBaseUrl, author }) {
  const url = `${workerBaseUrl}/images/${author}`;
  const res = await fetch(url);
  if (!res.ok) {
    const text = await res.text().catch(() => '');
